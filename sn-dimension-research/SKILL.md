...
## 搜索可用性探测（必须在广度发现后立即执行）

在广度发现过程中，记录每次 web_search 的返回结果。如果**连续 3 次不同查询均返回空结果**（即 web_search 调用成功但返回的搜索结果为空列表），立即判定当前环境的搜索能力不可用。此时：

1. **停止所有后续搜索尝试**。不要继续重试、更换搜索词或更换搜索入口——环境限制不是关键词问题。
2. **调整方法**：切换到基于训练知识的撰写模式。本 skill 不禁止在搜索不可用时使用训练知识；搜索的价值在于降低时效性风险和交叉验证，但缺失搜索时仍可基于已验证的成熟学术知识完成研究。
3. **在子报告中明确标注搜索状态**：在子报告顶部增加说明，如"因环境网络受限无法实时检索，近 X 年的人事变动、新设项目等可能存在更新滞后"。
4. **证据记录表标注来源层级**：所有基于训练知识的材料，在"可信度/限制"列标注"训练知识，未经验证"。
5. **充分性判断中主动承认缺口**：明确标注哪些原本应通过联网搜索确认的事实（最新政策、人事、数据）无法核实。

判定搜索不可用后，跳过后续的专项深挖和原始核验步骤，直接进入整合判断（执行流程第 9 步）。

## 并行子代理的注意事项

如果当前维度在 delegate_task 子代理中执行，注意：

1. **子代理无法继承父会话的 web_search 配置**。即使父会话的 web_search 可用，子代理可能因上下文隔离而无法访问同一 provider。在 step 3 后立即执行上述搜索可用性探测，不要盲目假设子代理环境与父代理一致。
2. **避免 JSON 竞态破坏**：多个子代理同时修改 `plan.json`（如各更新自己的 `status`）可能导致 JSON 格式损坏。建议子代理只写入 `sub_reports/{dimension_id}.md`，由父代理在全部子代理完成后统一重写 `plan.json` 中的全部 status 字段。
3. **超时检测**：如同一个维度在 delegate_task 中连续超时 2 次（均为 600s 超时），不再第 3 次重试，改为在父代理中直接用训练知识撰写该维度子报告。
...
3. **广度发现**：先从 `seed_queries` 开始，用通用搜索和必要的初始入口摸清这个维度的来源地图；记录新实体、术语、机构、年份、争议点和候选来源类型。**同时执行搜索可用性探测**（见上节）。
4. **选择深挖方向**：判断哪些来源类型最可能回答 `key_questions`，再决定是否调用专门搜索 skill 深挖。**如果搜索不可用已判定，跳过 4-7 步，直接进入第 9 步整合判断。**
...
- **环境搜索不可用时无脑重试**。连续 3 次 web_search 返回空结果仍继续增大查询深度，不做搜索不可用的判定和降级处理。应尽早检测、尽早降级、尽早记录限制。
- **子代理竞态破坏 plan.json**。多个 delegate_task 子代理同时 patch 同一个 JSON 文件时，若未统一协调，可能导致 JSON 格式损坏。应让子代理只写入子报告，由父代理在全部返回后统一重写 plan.json 的 status 字段。
