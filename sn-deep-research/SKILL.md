...
- 探测成功且返回非空结果即可继续。
- 探测失败、工具不存在、返回缺 key、provider 未就绪、服务不可达、search disabled、权限不足或结果为空时，标记为"搜索不可用环境"。此时**允许降级进入离线研究模式**，而不是硬性停止。

**离线研究模式规则**：
- 在 `request.md` 和 `report.md` 顶部均标注"本报告基于训练知识撰写，因环境搜索能力不可用，近 [X] 年数据可能存在更新滞后"
- 所有子报告的"不确定性与空白"必须明确列出因搜索不可用而无法验证的内容（最新人事变动、政策更新、市场数据等）
- 不使用任何需要联网验证的时效性数据

如果用户环境在后续可修复搜索能力（配置 provider、换模型等），优先恢复搜索再开始研究。只有确认搜索无法修复时，才进入离线研究模式。
...
**可选用 delegate_task 并行提速**：对于无依赖的维度（`depends_on` 为空），可同时派发给多个子代理并行执行，但注意：

- 限制并发数：受 `delegation.max_concurrent_children` 限制（通常 3 个），分批派发
- **超时降级**：如果某维度子代理连续超时 2 次（均因 web_search 无结果导致），由父代理直接用训练知识撰写该维度子报告，不再第 3 次提交
- **避免 JSON 竞态**：不要让子代理修改 `plan.json`。改为子代理只写入 `sub_reports/{dimension_id}.md`，父代理在全部维度完成后统一重写 `plan.json` 中的所有 `status` 字段
- **子代理解释性标注不足**：子代理在搜索不可用环境下可能不会主动标注限制。父代理需在收齐后统一检查各子报告的"不确定性与空白"节，对缺漏的补充标注
...
不要把报告章节直接当维度来写，也不要凭单轮搜索或少量材料直接成文。并行子代理取证的具体模式与坑参见 `references/subagent-parallel-pattern.md`。
...
**PDF 交付**：如果用户要求或习惯接收 PDF 格式，读取 `sn-research-report` 的"PDF 交付"章节，按引擎降级链（WeasyPrint > wkhtmltopdf > xelatex）执行转换。包含 Mermaid 图的报告需先做 ASCII 降级。

回复用户时至少提供：

- `report_dir` 的绝对路径
- `report.md` 的绝对路径
- 如果有 PDF 交付，提供 `report.pdf` 的路径
