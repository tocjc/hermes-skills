...
metadata:
  hermes:
    ...
    related_skills: [songwriting-and-ai-music, writing-beats]
---
...
## Article Structure: Beat-by-Beat Writing

When the user has raw material and wants to assemble it as a narrative rather than an argument, use the **beat-by-beat** method. A beat is one move in the journey — it does one thing (sets a scene, lands a point, asks a question, drops an aside, twists the angle), then stops.

### The process

1. From the raw material, write 2–3 candidate **starting beats**. Each is a different entry point. Show the user before writing to the file. The user picks one.
2. Write **only that beat** to the article file. A beat may be one sentence or several paragraphs — whatever that beat naturally is. Stop there.
...
- A single sentence if that's all the move requires ("And then nothing happened for three weeks.")
...
- If a beat needs five paragraphs and three subheadings, split it — it's two beats.
...
- The article ends when the journey is complete — not when the pile of raw material is empty.
- Pull material from the raw pile to populate the beat. Paraphrase, split, recombine, or quote. The pile is a quarry.
