...
当用户要求将报告导出为 PDF 时，按以下策略链依次尝试。每个步骤都在检测到工具不可用时优雅降级。

### ⚠️ 核心坑点：WeasyPrint 不嵌入 CJK 字体

WeasyPrint 渲染中文 PDF 时**依靠系统字体渲染**，但生成的 PDF 文件本身**不嵌入中文字体子集**。这意味着在缺少对应字体的 PDF 阅读器（Windows 默认预览器、部分手机阅读器）上，中文显示为乱码/方框。即使 CSS 中用了 `@font-face` 指定本地 `.ttc` 文件，WeasyPrint 也不会将字体子集嵌入 PDF。

**结论**：涉及中文内容的 PDF，**不要依赖 WeasyPrint**。优先使用 pandoc + xelatex。

### PDF 引擎选择（降级顺序）

| 优先级 | 引擎 | 安装方式 | 中文支持 | 适用场景 |
|-------|------|---------|---------|---------|
| **1** | **pandoc + xelatex**（首选） | `apt install pandoc texlive-xelatex texlive-lang-chinese` | **优秀（CID 字体子集嵌入 PDF）** | **中文文档首选**；texlive 体积约 300-500MB |
| 2 | **WeasyPrint**（仅英文或字体安全的纯文字环境） | `pip3 install weasyprint` | **需确认**（不嵌入字体，仅用于阅读器已有对应字体的场景） | 英文 PDF；或确认阅读器环境后使用 |
| 3 | **wkhtmltopdf** | `apt install wkhtmltopdf` | 需 CJK 字体 + 0.12.6 无 patch 版页眉页脚缺失 | 轻量临时使用，不推荐正式交付 |

### 中文关键准备

```bash
# 安装中文字体（任一即可）
apt-get install -y fonts-noto-cjk           # 推荐
# 或
apt-get install -y fonts-wqy-microhei       # 轻量备选
# 验证字体生效
...
Mermaid fenced code blocks 不会被 PDF 引擎原生渲染，必须提前处理：

1. **尝试渲染**：将 Mermaid 定义提取到 `.mmd` 文件，用 `@mermaid-js/mermaid-cli` 转 SVG
2. **失败降级**：如果渲染引擎不可用或超时，将 Mermaid flowchart 替换为 **ASCII 流程图**（用 `┌─┐│└─┘→` 等 Unicode 框线字符绘制）
3. **终极降级**：用一段结构化文字描述替代，列出节点和连接关系

处理流程：`report.md` → 提取 ````mermaid` 块到 `.mmd` 文件 → `npx mmdc` 转 SVG → 失败则替换为 ASCII 图

### 标准转换流程（pandoc + xelatex 法，中文首选）

```bash
# 确保字体和引擎已安装
apt-get install -y pandoc texlive-xelatex texlive-lang-chinese fonts-noto-cjk

# 一步转换：pandoc 直接调用 xelatex 生成 PDF
pandoc report.md \
  -f markdown+smart \
  --pdf-engine=xelatex \
...
**关键参数说明**：
- `-V mainfont` / `-V CJKmainfont`：指定系统中已安装的 CJK 字体名（`fc-list :lang=zh` 查到）
- `-V geometry:margin=2.5cm`：页边距
- 生成的 PDF 会将字体子集以 CID 格式嵌入，任何 PDF 阅读器都能正确显示中文

**注意**：如果源文件中包含 emoji 或变体选择符（U+FE0F），xelatex 会报 Missing character 警告，不影响正文输出。

### 标准转换流程（WeasyPrint 法，仅英文/无字体安全要求时使用）

```bash
# 1. 处理 Mermaid（替换为 ASCII 图或文字描述）
# 2. 用 pandoc 生成 HTML
pandoc report.md --to html5 --self-contained -o report.html
# 3. 注入 CSS + weasyprint 渲染 PDF
python3 -c "
import weasyprint
with open('report-style.css') as f: css = f.read()
with open('report.html') as f: html = f.read()
html = html.replace('</head>', f'<style>{css}</style>\n</head>')
with open('report_styled.html', 'w') as f: f.write(html)
weasyprint.HTML(filename='report_styled.html').render().write_pdf('report.pdf')
"
```

### 参考 CSS

本 skill 在 `references/report-pdf-style.css` 提供了一份专用于学术中文报告的 PDF 样式表。包含表格斑马纹、章节标题层级配色、引用块样式和 A4 纸张设置。使用前确认该文件存在。

### 常见失败

- 无 patch 版 wkhtmltopdf 在无 X server 环境报错 → 改用 WeasyPrint
- Mermaid CLI 超时 → 无 puppeteer 浏览器，进入 ASCII 降级
- 中文字体缺失（PDF 中文字显示为方框）→ 先 `fc-list :lang=zh` 确认，再安装 fonts-noto-cjk
