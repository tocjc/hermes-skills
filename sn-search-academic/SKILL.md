...
- **Semantic Scholar 公开 API** 无 key 时约 1 req/s 限速，批量调用易得空响应（脚本输出为空或 parse error）。传 `S2_API_KEY` 环境变量可提升限额。空响应时先重试一次，仍空则换 `web-access` 浏览器工具或 arXiv。
- **PubMed** 无 key 时限额 3 req/s，传 `NCBI_API_KEY` 可升至 10 req/s。
- **Wikipedia** 无速率限制，但中国国内访问可能不稳定。
...
> **参考文件**：`references/search-fallback-strategies.md` — 当标准学术搜索工具（Semantic Scholar、Bing、Baidu 等）因网络限流或反爬失败时的降级策略，包括直接访问大学官网、GitHub API 替代方案等。
